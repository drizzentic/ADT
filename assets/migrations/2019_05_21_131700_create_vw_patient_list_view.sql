...
AS 
  SELECT p.id 
         ...
         p.dob 
            AS date_of_birth, 
         Concat(Timestampdiff(year, p.dob, Now()), ' years ', 
         Timestampdiff(month, p.dob, Now()) % 12, ' months') 
            AS age, 
         IF(Round (Datediff(Curdate(), p.dob) / 365) >= 15, 'Adult', 
         'Paediatric') 
            AS 
         maturity, 
         p.pob 
         ...
         (SELECT patient_viral_load.test_date 
          FROM   patient_viral_load 
          WHERE  ( patient_viral_load.patient_ccc_number = 
                 p.patient_number_ccc ) 
          ORDER  BY patient_viral_load.test_date DESC 
          LIMIT  1) 
            AS viral_load_test_date, 
